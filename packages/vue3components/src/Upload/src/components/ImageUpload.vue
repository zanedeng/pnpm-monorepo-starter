...
import { isArray, isFunction, isObject, isString } from '@zkj/utils'
import { useI18n } from '@zkj/vue3locale'
import { PlusOutlined } from '../../../Icon'
import { useUploadType } from '../hooks/useUpload'
import { uploadContainerProps } from '../props'
import { isImgTypeByName } from '../helper'
import { UploadResultStatus } from '../../../Upload/src/types/typing'
import type { UploadFile, UploadProps } from 'ant-design-vue'
import type { UploadRequestOption } from 'ant-design-vue/lib/vc-upload/interface'

defineOptions({ name: 'ImageUpload' })

const emit = defineEmits(['change', 'update:value', 'delete', 'message'])
...
const handlePreview = async (file: UploadFile) => {
  ...
  previewTitle.value =
    file.name ||
    previewImage.value.substring(previewImage.value.lastIndexOf('/') + 1)
}
...
const beforeUpload = (file: File) => {
  ...
  if (!isAct) {
    emit('message', {
      type: 'error',
      content: t('component.upload.acceptUpload', [accept]),
    })
